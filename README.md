...
NaviDash 专注于你打开浏览器后的第一站：打开常用链接、快速发起搜索、记录临时想法，或者确认本地与自托管服务是否正常。它不是另一个完整监控平台、RSS 阅读器或项目管理系统，而是一个克制、可定制、适合长期使用的个人工作台。

## 为什么做 NaviDash

很多导航页项目很容易走向两端：要么只是漂亮的链接集合，要么变成越来越重的 dashboard。NaviDash 希望站在中间：保留个人首页该有的美观和自由布局，同时把核心体验收束到每天真的会用的动作上。

- **更快打开**：把常用链接、项目入口和本地服务放在一处。
- **更快搜索**：在首页直接搜索，并支持从已保存链接中快速命中。
- **更快记录**：用轻量便签承接临时信息，而不是打开完整文档工具。
- **更可控**：支持 Docker 自托管，数据保存在你自己的机器或 NAS 上。
- **更有边界**：优先做好启动页体验，不试图替代专业工具。
...
这些能力是接下来重点考虑的技术向扩展，目标是保持“启动页视角”，只展示关键状态和最快入口。
...
路线图会刻意保持克制：相比不断堆组件，NaviDash 更优先打磨每天都会用到的动作。
