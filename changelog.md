...
### docs: 将最近更新放到 README 首屏
- 做了什么：在中英文 README 的项目封面后增加最近更新区块，提炼文档站、F1 积分、Demo 和部署文档等最新进展，并保留完整 changelog 入口
- 影响范围：GitHub 项目首页的信息时效性和首次浏览路径
- 涉及模块：`README.md`、`README_EN.md`、`changelog.md`
- 是否有兼容性影响：无；仅调整公开文档结构
- 后续待补充：后续发布重要功能或文档变化时同步更新 README 最近更新区块

## 2026-08-29

### docs: 将文档站调整为使用与配置指南
- 做了什么：缩小文档站正文、标题和移动端字号，将首页从产品介绍改为首次使用与常用任务入口；新增可直接复制的完整 `.env` 示例，以及访问密码、天气、Komari、F1 数据与 Demo 模式的集中配置页，并细化书签、布局和各 Widget 的实际操作步骤
- 影响范围：文档站信息层级、排版密度、用户部署后的配置与使用路径
- 涉及模块：`docs-site/index.md`、`docs-site/guide/`、VitePress 导航与主题样式、`README*.md`、`.env.example`
- 是否有兼容性影响：无；仅调整文档内容与展示，不修改 NaviDash 运行行为或部署链路
- 后续待补充：根据用户实际配置过程继续补充截图和特定 NAS 平台示例

### feat: 新增独立 VitePress 文档站
- 做了什么：新增中文优先的 VitePress 文档站，整理局域网部署、日常使用、Widget、升级备份和常见问题页面；配置本地搜索、深色模式、移动端布局和 GitHub Pages 自动发布工作流
- 影响范围：项目文档浏览与发布流程
- 涉及模块：`docs-site/`、`.github/workflows/docs-site.yml`、`README*.md`、`.gitignore`
- 是否有兼容性影响：无；文档站使用独立依赖和静态构建，不改变 Next.js 应用、Docker 发布或 Vercel 自动部署
- 后续待补充：在仓库设置中将 GitHub Pages 发布源设为 GitHub Actions，并根据实际访问反馈补充页面内容

### docs: 聚焦个人局域网部署与使用
- 做了什么：新增精简 Wiki 入口，并将面向用户的部署文档重心调整为 Docker 持久化、局域网访问、固定主机地址、备份恢复和常见排障；项目自身的在线演示与自动部署配置保持不变，同时补充 F1 积分与 Komari 的使用说明
- 影响范围：项目首页、部署指南、用户指南
- 涉及模块：`README*.md`、`docs/WIKI.md`、`docs/DEPLOY.md`、`docs/USER_GUIDE*.md`
- 是否有兼容性影响：无；仅更新用户文档，不改变自动部署、运行配置和数据结构
- 后续待补充：根据个人电脑、NAS 和家庭服务器的实际部署反馈继续完善局域网排障项
...
- 后续待补充：后续新增核心 Widget 时同步评估是否加入 Demo，并定期更新演示积分数据
