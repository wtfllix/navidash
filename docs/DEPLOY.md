...
本文档面向个人局域网中的长期自托管：在家用电脑、NAS 或小型服务器上运行 NaviDash，
由同一网络中的手机和电脑访问。推荐使用 Docker Compose，并把运行数据持久化到宿主机。

NaviDash 会在服务端保存布局和配置，因此需要一个长期在线且具有可写磁盘的运行环境；
无持久化文件系统的临时或无状态托管不适合作为个人正式实例。
...
在结果中找到常见为 `192.168.x.x`、`10.x.x.x` 或 `172.16-31.x.x` 的 IPv4 地址。
同一局域网中的设备使用这个地址访问：
...
建议在路由器中为部署主机保留固定 DHCP 地址，避免主机 IP 变化后浏览器书签失效。

## 可选：远程访问

局域网内使用不要求配置域名。如果需要从外网访问，优先通过可信 VPN 回到家庭网络。
确实要使用公网域名时，再使用 Caddy、Nginx、Traefik 等反向代理提供 HTTPS，并设置
`NAVIDASH_ACCESS_PASSWORD`；不要直接把 `3000` 端口暴露到公网。
...
- 赛程来自应用内置数据，不依赖外部接口。
- 车手积分由服务端读取 Jolpica，成功结果缓存 24 小时；部署主机需要能够访问 `https://api.jolpi.ca`。
- 上游暂时失败时会继续使用最近一次成功缓存；首次请求且没有缓存时才会显示不可用状态。
